...
def collect_tokens(token, tokens):
...
def create_tables_from_child_queries(user, connection, query, child_queries):
    for i, child_query in enumerate(child_queries):
        ...
        _rendered_child_query = pystache.render(
            _child_query.query_text, params)
        ...
        if error:
            raise ChildQueryExecutionError(
                'Failed loading results for query id {}.'.format(i, _child_query.id))
...
class ResultsWithParams(Results):
    ...
    def run_query(self, query, user):
        ...
        try:
            ...
            query = create_tables_from_child_queries(
